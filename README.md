...
*   **번역 평가 도구**: 원본 이미지와 번역된 이미지(또는 조각)를 비교하여 품질을 검증하는 `evaluator.py` 모듈을 제공합니다 (Gemini 3.1 Pro 모델 사용).
...
*   `evaluator.py`: 번역된 이미지 조각의 품질을 원본과 비교 평가하는 모듈입니다 (Gemini 3.1 Pro 사용).
